...
-- 21.	Tạo khung nhìn có tên là v_nhan_vien để lấy được thông tin của tất cả các nhân viên có địa chỉ là “Huế” và đã từng lập hợp đồng cho một hoặc nhiều khách hàng bất kì với ngày lập hợp đồng là “01/06/2021”.
CREATE view v_nhan_vien as
SELECT NV.ma_nhan_vien, NV.ho_ten, NV.so_dien_thoai, NV.email, NV.dia_chi FROM nhan_vien NV
JOIN hop_dong HD 
ON NV.ma_nhan_vien = HD.ma_nhan_vien
WHERE NV.dia_chi LIKE '%Huế%' AND HD.ngay_lam_hop_dong = '2021-06-01'
...
DELIMITER / / CREATE PROCEDURE sp_them_moi_hop_dong (
    id int,
    ngay_lam_hop_dong_add datetime,
    ngay_ket_thuc_add datetime,
    tien_dat_coc_add DOUBLE,
    ma_nhan_vien_add int,
    ma_khach_hang_add int,
    ma_dich_vu_add int
) BEGIN IF id NOT IN (
    SELECT
        HD.ma_hop_dong
